          ...
          <div class="check-group">
            <el-radio-group v-model="checkValue">
              <div v-for="(item, index) in list"
                :key="index"
                class="groupbox">
                <el-radio :label="item.id">{{item.spName}}</el-radio>
              </div>
          ...
          <div class="check-group">
            <el-checkbox-group v-model="checkList"
              ...
              <div v-for="(item, index) in list"
                :key="index"
                class="groupbox">
                <el-checkbox :label="item.id">{{item.spName}}</el-checkbox>
              </div>
              <div style="clear: both;"></div>
            </el-checkbox-group>
          </div>
...
export default {
  ...
  methods: {
    ...
    handleQuery(currentPage, pageSize, data) {
      let apiUrl = null
      if (this.multiple === 'subject') {
        apiUrl = allApi.serviceproject.getList
        ...
        apiUrl = allApi.serviceprovider.allList
        getRecordList(apiUrl, {}).then(response => {
          console.log(response)
    ...
    update() {
      const checkName = this.list.reduce((res, item) => {
        if (this.checkList.find(v => v === item.id)) {
          return res.concat(item.id + '-' + item.spName)
        }
        return res
      }, [])
      ...
      if (this.multiple !== undefined) {
        ...
        if (kkk) this.checkValue = this.checkValue + '-' + kkk.spName
        this.$emit('setGoback', this.checkValue)
      }
    },
    cancel() {
