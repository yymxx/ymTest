...
export default {
  ...
  methods: {
    ...
    handleQuery(currentPage, pageSize) {
      ...
      const apiUrl = allApi.materMeans.getList
    ...
    create(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          const apiUrl = allApi.materMeans.addData
    ...
    update(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.dialogFormVisible = false
          const apiUrl = allApi.materMeans.updateData
    ...
    handleDelete() {
      if (this.mulSelection.length > 0) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          const apiUrl = allApi.materMeans.delData
