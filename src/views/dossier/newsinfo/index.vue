...
export default {
  ...
  methods: {
    findDicList() {
      findByDicCode('contNClass').then(response => {
    ...
    handleQuery(currentPage, pageSize) {
      ...
      const apiUrl = allApi.newsinfo.getList
      getRecordList(apiUrl, {
        ...
        contNClass: this.listQuery.contClass
      }).then(response => {
    ...
    create(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          const apiUrl = allApi.newsinfo.addData
    ...
    handleUpdate() {
      ...
      if (this.mulSelection.length === 1) {
        ...
        this.form.contNClass = parseInt(this.form.contNClass)
        this.form.contents = ''
      } else {
        this.$message({
          type: 'warning',
    ...
    update(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.dialogFormVisible = false
          const apiUrl = allApi.newsinfo.updateData
    ...
    handleDelete() {
      if (this.mulSelection.length > 0) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          const apiUrl = allApi.newsinfo.delData
