<template>
  <div class="main-container">
    ...
    <el-table border
      ...
      <el-table-column label="慢性情况"
        prop="loseAbility"
        align="center">
      </el-table-column>
      <el-table-column label="失能情况"
        prop="disability"
        align="center">
      </el-table-column>
      <el-table-column label="残疾情况"
        prop="serviceState"
        align="center">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleQuery(currentPage, pageSize) {
      ...
      const apiUrl = allApi.healthy.getList
    ...
    create(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          const apiUrl = allApi.healthy.addData
    ...
    update(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.dialogFormVisible = false
          const apiUrl = allApi.healthy.updateData
    ...
    handleDelete() {
      if (this.mulSelection.length > 0) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          const apiUrl = allApi.healthy.delData
